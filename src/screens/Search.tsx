...
import { View, TextInput, Button, FlatList, Text, Image, StyleSheet, Alert, ScrollView, Dimensions, Modal, TouchableOpacity } from 'react-native';
...
const PEXELS_API_FEATURED_URL = 'https://api.pexels.com/v1/collections/featured';
...
const SearchScreen = () => {
    ...
    return (
        ...
        <Text style={styles.featuredText}>{keyword ? `${keyword}` : `Curated`} Photos</Text>

        <ScrollView ref={scrollViewRef} contentContainerStyle={styles.photoGrid}>
                    {columns.map((columnPhotos, columnIndex) => (
                        <View key={columnIndex} style={styles.column}>
                            {columnPhotos.map((photo) => (
                                <TouchableOpacity key={photo.id} onPress={() => openPhotoModal(photo)}>
                                    <PhotoCard key={photo.id} photo={photo} columnWidth={columnWidth} />
                                </TouchableOpacity>
                            ))}
                        </View>
                    ))}
